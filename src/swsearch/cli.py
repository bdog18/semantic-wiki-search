...
def pipeline(
    with_triplets: bool = typer.Option(
        ...
        "(slow; only needed to train a model with `swsearch train-transfer` -- search/evaluate don't use them)."
    ),
    ...
    index_type: str = typer.Option("auto", help="Index type: 'auto' (flat for small corpora, IVF-PQ for large), 'flat' or 'ivfpq'."),
) -> None:
...
def build_index(
    ...
    if index_type not in ("auto", "flat", "ivfpq"):
        raise typer.BadParameter("index_type must be 'auto', 'flat' or 'ivfpq'")

    meta_conn = load_faiss_meta_sqlite(str(meta_db))
    try:
        index = build_index_from_manifest(str(embeddings_dir), meta_conn, str(index_path), index_type=index_type)
    finally:
        meta_conn.close()
    typer.echo(f"FAISS index ({type(index).__name__}, {index.ntotal} vectors) written to {index_path}")
...
def train_transfer(
    ...
    checkpoint scores best -- on real retrieval MRR@10 when --eval-meta-db is
    given, and on held-out triplet accuracy only as a fallback. Reuses the
    triplets mined once against the baseline index -- no separate mining run
    is needed for this. The saved model can then be passed to
...
def evaluate(
    ...
    k_values: str = typer.Option("1,3,5,10", help="Comma-separated K values for Top-K Accuracy."),
    ...
    """Report MRR, Mean R-Precision, MAP and Top-K Accuracy against a real
    SearchEngine. Each query is searched once and every metric scores that
    same ranked list."""
